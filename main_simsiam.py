...
class Worker:
    def __call__(self, origargs):
        ...
        if origargs.model.q_as_prototype:
            from main_prototype_worker import main_worker
        else:
            from main_simsiam_worker import main_worker
...
def main(args):
    ...
    args.logging.name = args.logging.name.replace('${model.pr_queue_size}', str(args.model.pr_queue_size))
